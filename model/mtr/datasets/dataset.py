...
import model.mtr.utils.common_utils as common_utils
    ...
    def collate_batch(self, batch_list):
        ...
        for key, val_list in key_to_list.items():
            ...
            if key in ['obj_trajs', 'obj_trajs_mask', 'map_polylines', 'map_polylines_mask', 'map_polylines_center',
                'obj_trajs_pos', 'obj_trajs_last_pos', 'obj_trajs_future_state', 'obj_trajs_future_control',
            ...
            elif key in ['surrounding_obj_trajs_full', 'surrounding_obj_trajs_valid_mask',
                         'all_agent_reference_xy_heading',
                         'all_agent_reference_xy_heading_center',
                         'all_agent_reference_xy_heading_normalized']:
                # TODO: should be similar to obj_traj_future_state, center_num and all_agent_num/surrounding_agent_num can both be different across scenario
                val_list = [torch.from_numpy(x) for x in val_list]
                input_dict[key] = common_utils.merge_batch_by_padding_2nd_dim(val_list)
            elif key in ['surrounding_obj_index','surrounding_obj_id']:
                # TODO: This should be list of list [[]], contains the index of surrounding agents for each center agents.
                #  Similar to tracks to predicts, since we will be merging with the maximum number of surrounding agents,
                #  so it's okay to keep the indices here
                merge_list = []
                for sublist in val_list:
                    merge_list.extend(sublist)
                input_dict[key] = merge_list
            else:
                val_list = [torch.from_numpy(x) for x in val_list]
