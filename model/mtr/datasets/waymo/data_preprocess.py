...
def decode_map_features_from_proto(map_features):
    ...
    for cur_data in map_features:
        ...
        if cur_data.lane.ByteSize() > 0:
            ...
            # There are some id that doesn't have polyline at all, so continue loop without raising error
            continue
...
def process_waymo_data_with_scenario_proto(data_file, output_path=None):
    with tf.device('/CPU:0'):
        ...
        for cnt, data in enumerate(dataset):
            ...
            # With info, we can get track information and save in track_infos
...
def validate_data_path(path_input, path_type="data"):
    """
    Validate and sanitize data paths to prevent path traversal attacks.
    
    Args:
        path_input: User-provided path
        path_type: Type of path ("data" or "output") for error messages
    ...
    try:
        ...
        raise ValueError(f"Invalid {path_type} path: {path_input}") from e
    
    # Check for path traversal attempts in the original input
    if '..' in str(path_input):
        raise ValueError(f"Path traversal detected in {path_type} path: {path_input}")
    
    # For data paths, ensure they exist
    if path_type == "data":
        if not validated_path.exists():
            raise ValueError(f"Data path does not exist: {validated_path}")
        if not validated_path.is_dir():
            raise ValueError(f"Data path must be a directory: {validated_path}")
    
    # For output paths, ensure parent directory exists or can be created
    # and that we're not writing to sensitive system directories
    if path_type == "output":
        # Disallow writing to system directories
        sensitive_dirs = ['/etc', '/bin', '/sbin', '/usr', '/boot', '/sys', '/proc']
        for sensitive in sensitive_dirs:
            try:
                validated_path.relative_to(sensitive)
                raise ValueError(f"Cannot write to system directory: {validated_path}")
            except ValueError as e:
                if "not in the subpath" not in str(e) and "not a subpath" not in str(e):
                    raise
...
def create_infos_from_protos(raw_data_path, output_path, num_workers=16):
    # SECURITY FIX: Validate paths to prevent path traversal attacks
    raw_data_path = validate_data_path(raw_data_path, path_type="data")
    output_path = validate_data_path(output_path, path_type="output")
    
    train_infos = get_infos_from_protos(
        data_path=str(raw_data_path / 'training'),
        output_path=str(output_path / 'processed_scenarios_training'),
        num_workers=num_workers
    )
    train_filename = str(output_path / 'processed_scenarios_training_infos.pkl')
    with open(train_filename, 'wb') as f:
        pickle.dump(train_infos, f)
    print('----------------Waymo info train file is saved to %s----------------' % train_filename)

    val_interactive_infos = get_infos_from_protos(
        data_path=str(raw_data_path / 'validation_interactive'),
        output_path=str(output_path / 'processed_scenarios_validation_interactive'),
        num_workers=num_workers
    )
    val_interactive_filename = str(output_path / 'processed_scenarios_validation_interactive_infos.pkl')
    with open(val_interactive_filename, 'wb') as f:
        pickle.dump(val_interactive_infos, f)
    print('----------------Waymo info interactive file is saved to %s----------------' % val_interactive_filename)
